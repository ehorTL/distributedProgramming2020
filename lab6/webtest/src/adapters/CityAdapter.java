...
public class CityAdapter {
    ...
    public static ArrayList<Pair<City, Country>> getAllCitiesWithCountries(Connection connection) throws SQLException {
        ArrayList<Pair<City, Country>> result = new ArrayList<>();
        String q = "SELECT * from city join country on city.country_id=country.country_id";
        ResultSet resultSet = connection.createStatement().executeQuery(q);
        while(resultSet.next()){
            City cityParsed = new City(
                    resultSet.getString("city_name"),
                    resultSet.getString("city_code"),
                    resultSet.getInt("city_id"),
                    resultSet.getLong("city_population"),
            ...
            Country countryParsed = new Country(
                    resultSet.getString("country_code"),
                    resultSet.getString("country_name"),
                    resultSet.getInt("country_id")
            );
            result.add(new Pair<City, Country>(cityParsed, countryParsed));
        }
        return result;
    }
